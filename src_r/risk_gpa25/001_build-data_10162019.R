...
# I'll use median
ct <- xf.trs.courses %>% group_by(dept_abbrev) %>% summarize(nd = n_distinct(system_key)) %>% summarize(median(nd)) %>% deframe()
...
stu.dept.data.wide <- xf.trs.courses %>%
  ...
  group_by(dept_abbrev) %>%
  # reduce total - may want to choose a different method
